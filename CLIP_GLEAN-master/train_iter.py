...
from src.models.iter_optimization import create_model
from src.data import create_datamodule

config_parser = parser = argparse.ArgumentParser()

parser.add_argument('-c', '--config', 
                    default = 'configs/train_iter/clipgpen_iter_0_clip.yaml', 
                    type    = str, 
                    metavar = 'FILE',
                    help    = 'YAML config file specifying default arguments')

parser = argparse.ArgumentParser(description='CLIP-GPEN Iterative Optimization')
...
args.data['train']['image_id'] = '%05d'%(args.img) if args.img != -1 else args.data['train']['image_id']
args.data['train']['caption'] = args.text if args.text != '' else args.data['train']['caption']
args.data['test']['image_id'] = '%05d'%(args.img)if args.img != -1 else args.data['test']['image_id']
args.data['test']['caption'] = args.text if args.text != '' else args.data['test']['caption']
args.name = '%s_%s_%s'%(args.data['train']['image_id'], args.data['train']['caption'], args.name)
...
def main():
    ...
    args.save_path = args.network['save_path'] = osp.join('experiments/train_iter', args.name)
