...
class LitClipGpen(LightningModule):
    def __init__(self,
        ...
        for p in self.generator.image16_mapper.parameters():
            p.requires_grad=True
        for p in self.generator.generator.latent_mapper.parameters():
            p.requires_grad=True
    ...
    def training_step(self, batch, batch_idx):
        ...
        tokenized_caption = torch.cat([clip.tokenize(cap) for cap in caption]).cuda()
        text_features = self.clip_model.encode_text(tokenized_caption)
        fake_g_output, w, image16_code, latent_code = self.generator(lq, text_features, strategy=['image16', 'latent'])
        glean_g_output, w_glean , _, _ = self.generator(lq, strategy=[])
        ...
        if self.loss_gan:
            ...
            fake_g_pred = self.discriminator(F.interpolate(fake_g_output,size=(1024,1024),mode='bicubic', antialias=True))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        #gpen
        test_img, _, _, _ = self.generator(lq, text_features, strategy=['image16', 'latent']) 
