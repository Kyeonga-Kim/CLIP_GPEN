...
from src.models.mmgen.models.architectures.stylegan import StyleGANv2Generator
from src.models.latent_mapper import CondMapper, Mapper, TempMapper, TextMapper


@BACKBONES.register_module()
class CLIPGLEANStyleGANv2(nn.Module):
    ...
    def __init__(self,
        ...
        caption_out_dim = num_styles * style_channels
        # self.caption_mapper = nn.Sequential(
        #     # nn.Linear(512, caption_out_dim),
        #     nn.Linear(caption_out_dim+512, caption_out_dim),
        #     nn.LeakyReLU(negative_slope=0.2, inplace=True),
        #     nn.Linear(caption_out_dim, caption_out_dim),
        #     nn.LeakyReLU(negative_slope=0.2, inplace=True),
        #     nn.Linear(caption_out_dim, caption_out_dim)
        # )
        # self.caption_premapper = Mapper(None)
        # self.caption_mapper = TempMapper(None,cond_dim=512,latent_dim=512)
        # self.caption_mapper = TextMapper(
        #     num_style_block=num_styles,
        #     mapper_style='CondMapper')
        # self.caption_mapper = TextMapper(
        #     num_style_block=num_styles,
        #     mapper_style='Mapper')
        ...
        for res in decoder_res:
            if res == in_size: 
                in_channels = channels[res] # original
            else:
                in_channels = 2 * channels[res] # original
...
#                         in_channels, out_channels, 2, upsample_kernel=3))
        ...
        self.delta_image = torch.nn.Parameter(torch.zeros((1,3,16,16), requires_grad=True), requires_grad=True)
        self.delta_code = torch.nn.Parameter(torch.zeros((1,num_styles,style_channels), requires_grad=True), requires_grad=True)

    def _get_delta_image(self, strategy):
        return self.delta_image if 'image_space' in strategy else torch.zeros_like(self.delta_image)

    def _get_delta_code(self, strategy):
        return self.delta_code if 'code_space' in strategy else torch.zeros_like(self.delta_code)

    def forward(self, lq, strategy=['image_space', 'code_space']):
        """Forward function.
        Args:
        ...
        # Encoder
        feat = lq + self._get_delta_image(strategy)
        encoder_features = []
        ...
        caption_code = self._get_delta_code(strategy)
        out = self.generator.conv1(out, latent[:, 0] + caption_code[:, 0], noise=injected_noise[0])
        skip = self.generator.to_rgb1(out, latent[:, 1] + caption_code[:, 1])
        ...
        for up_conv, conv, noise1, noise2, to_rgb in zip(
            ...
            out = up_conv(out, latent[:, _index] + caption_code[:, _index], noise=noise1)
            out = conv(out, latent[:, _index + 1] + caption_code[:, _index + 1], noise=noise2)
            skip = to_rgb(out, latent[:, _index + 2] + caption_code[:, _index + 2], skip)
        ...
        for i, block in enumerate(self.decoder):
            # cap = caption.repeat((1,1) + hr.shape[2:])
            if i > 0:
                hr = torch.cat([hr, generator_features[i - 1]], dim=1) # original   
            # hr = torch.cat([hr, cap], dim=1)
            hr = block(hr)

        return hr, latent, caption_code
