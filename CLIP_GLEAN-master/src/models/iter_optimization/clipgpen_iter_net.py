...
class Generator(nn.Module):
    ...
    def forward(
        ...
        if caption_code != None : 
            out = self.input(latent)
            out = self.conv1(out, latent[:, 0] + caption_code[:, 0], noise=noise[0])
            skip = self.to_rgb1(out, latent[:, 1] + caption_code[:, 1])

            i = 1
            for conv1, conv2, noise1, noise2, to_rgb in zip(
                self.convs[::2], self.convs[1::2], noise[1::2], noise[2::2], self.to_rgbs
            ):
                out = conv1(out, latent[:, i] + caption_code[:, i], noise=noise1)
                out = conv2(out, latent[:, i + 1] + caption_code[:,  i + 1], noise=noise2) #latent : (1,14,512)? #caption_code : (1,8,512)
                skip = to_rgb(out, latent[:, i + 2] + caption_code[:,  i + 2] , skip)
...
class FullGeneratorCLIP(nn.Module):
    def __init__(
        self,
        size,
        style_dim=512,
        ...
        self.image16_code = torch.nn.Parameter(torch.zeros((1, 3, 16, 16), requires_grad=True), requires_grad=True)
        self.image256_code = torch.nn.Parameter(torch.zeros((1, 3, 256, 256), requires_grad=True), requires_grad=True)
        self.caption_code = torch.nn.Parameter(torch.zeros((1, self.n_latent, style_dim), requires_grad=True), requires_grad=True)
    ...
    def _get_image16_code(self, strategy):
        return self.image16_code if 'image16_code' in strategy else torch.zeros_like(self.image16_code)
    
    def _get_image256_code(self, strategy):
        return self.image256_code if 'image256_code' in strategy else torch.zeros_like(self.image256_code)

    def _get_caption_code(self, strategy):
        return self.caption_code if 'caption_code' in strategy else torch.zeros_like(self.caption_code)
    ...
    def forward(self,
        ...
        truncation_latent=None,
        input_is_latent=False,
        strategy=['image16_code', 'caption_code']
    ):
        
        image16_code = image16_code if image16_code is not None else self._get_image16_code(strategy)
        image256_code = image256_code if image256_code is not None else self._get_image256_code(strategy)
        caption_code = caption_code if caption_code is not None else self._get_caption_code(strategy)

        inputs = inputs + image_code
        ...
        output, latent = self.generator([outs], caption_code, inject_index, truncation, truncation_latent, input_is_latent, noise=noise[1:])

        return output, latent, image_code, caption_code
