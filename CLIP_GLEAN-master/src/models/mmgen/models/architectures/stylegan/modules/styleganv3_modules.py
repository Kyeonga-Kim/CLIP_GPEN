...
from src.models.mmgen.models.builder import MODULES
from src.models.mmgen.ops import bias_act, conv2d_gradfix, filtered_lrelu
...
class SynthesisNetwork(nn.Module):
    ...
    def __init__(
        ...
        **layer_kwargs,
        ...
        #print(sampling_rates, cutoffs, half_widths)
        channels = np.rint(
            np.minimum((channel_base / 2) / cutoffs, channel_max))
        channels[-1] = self.img_channels
        #print('sizes and channels', sizes, channels)
        #input()
        self.channels = channels
        ...
        for idx in range(self.num_layers + 1):
            ...
            #print(name, is_torgb, int(sizes[prev]), int(sizes[idx]), is_critically_sampled)
        

    def forward(self, ws, glean, **layer_kwargs):
        """Forward function."""
        ...
        x = self.input(ws[0])
        #print(x.shape)
        for i, (name, w) in enumerate(zip(self.layer_names, ws[1:])):
            #print(i, 'bf', x.shape, w.shape)
            x = getattr(self, name)(x, w, **layer_kwargs)
            #print(i, 'af', x.shape, w.shape)
            if glean and i == self.num_layers-1:
                bank_feature.append(x)
